...
def main_parameters():
    ...
    parser.add_argument('--gamma', type=float, default=0.95, help='折扣因子') #0.99
    parser.add_argument('--buffer_capacity', type=int, default=int(1e6), help='经验回放缓冲区容量')
    parser.add_argument('--batch_size', type=int, default=1024, help='批次大小') #128
    parser.add_argument('--actor_lr', type=float, default=0.01, help='Actor学习率') #0.0002
    parser.add_argument('--critic_lr', type=float, default=0.01, help='Critic学习率') #0.002
    parser.add_argument('--comm_lr', type=float, default=0.0001, help='Comm学习率') #0.0001
    # 通信网络参数
